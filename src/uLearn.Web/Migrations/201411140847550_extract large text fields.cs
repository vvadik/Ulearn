namespace uLearn.Web.Migrations
{
    ...
    public partial class extractlargetextfields : DbMigration
    {
        public override void Up()
        {
			...
			const string updateFormat = "update dbo.UserSolutions set {0}Hash = convert(varchar(40), hashbytes('SHA1', {0}), 2) where {0} is not null";

			const string mergeFormat = "merge dbo.TextBlobs as texts "
									   + "using (select distinct {0}Hash, {0} from dbo.UserSolutions where {0} is not null) as solutions "
									   + "on (texts.Hash = solutions.{0}Hash) "
									   + "when not matched then insert values (solutions.{0}Hash, solutions.{0});";

			Sql("update dbo.UserSolutions set SolutionCodeHash = convert(varchar(40), hashbytes('SHA1', Code), 2)");
			Sql(String.Format(updateFormat, "CompilationError"));
			Sql(String.Format(updateFormat, "Output"));

			Sql("insert into dbo.TextBlobs select distinct SolutionCodeHash, Code from dbo.UserSolutions");
