...
{blocksXml}
			...
			var buildUpContext = new BuildUpContext(new FileSystem(new DirectoryInfo(".")), CourseSettings.DefaultSettings, null);
			return fileInfo.DeserializeXml<Lesson>().Blocks
				.SelectMany(
					b => b.BuildUp(buildUpContext, ImmutableHashSet<string>.Empty))
				.ToArray();
		}
	}
}