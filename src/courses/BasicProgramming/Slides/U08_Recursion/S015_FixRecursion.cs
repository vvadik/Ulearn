...
namespace uLearn.Courses.BasicProgramming.Slides.U08_Recursion
{
	...
	public class S015_FixRecursion : SlideTestBase
	{
		...
		Но что-то глупый C# отказывается правильно выполнять прекраснейшую программу Васи!
...
")]
		...
		public static void WriteReversed(char[] items, int startIndex = 0)
		{
			...
			/*uncomment
			WriteReversed(items, startIndex+1); // Выводим все элементы с индексом больше startIndex
			Console.Write(items[startIndex]); // и заканчиваем элементом с индексом startIndex
			*/
		}
	}
}