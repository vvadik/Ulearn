namespace uLearn.Courses.BasicProgramming.Slides.U06_AutomaticTesting
{
	...
	public class S099_Exercise
	{
		...
		Поля не могут быть пустыми, не могут содержать пробельных символов и разделяются одним или несколькими пробельными символами.

		### 2. Кавычки (+2 балла)

		Поля могут содержать пробелы и быть пустыми, но только если они заключены в двойные или одинарные кавычки.
		То есть строка `a "bcd ef"   'x y'` содержит три поля, а не пять.

		Кавычки разных типов могут быть вложенными. То есть строка `"a 'b' 'c' d" '"1" "2" "3"'` содержит два поля.
