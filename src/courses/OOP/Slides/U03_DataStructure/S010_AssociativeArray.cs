...
namespace OOP.Slides.U03_DataStructure
{
	...
	public class S010_AssociativeArray
	{
		...
		* remove(ключ)
		...
		Напомним основные факты про ассоциативные массивы.
		...
		Самая тривиальная реализация ассоциативного массива — это динамический массив пар (https://en.wikipedia.org/wiki/Dynamic_array). 
		...
		* remove — $O(N)$, так как для удаления элемента необходимо сначала найти удаляемую пару за $O(N)$, а затем удалить элемент из динамического массива за $O(N)$. 
		...
		Проблемой данной реализация является неэффективность операций FIND и REMOVE.
