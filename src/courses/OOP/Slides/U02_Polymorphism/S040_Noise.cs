...
namespace OOP.Slides.U02_Polymorphism
{
	...
	public class S040_Noise
	{
		...
		*/
		...
		1. Создайте интерфейс `INoise`, описывающий модель шума (то есть функцию $noise$). 
		...
		2. Создайте класс `UniformNoise`, реализующий `INoise` согласно следующей формуле:
			...
			$noise(x)=x\cdot	rnd(-w,w)$

			где $rnd(min,max)$ — генератор случайных действительных чисел в диапазоне от $min$ до $max$, а $w$ — параметр, передаваемый в конструктор `UniformNoise`
