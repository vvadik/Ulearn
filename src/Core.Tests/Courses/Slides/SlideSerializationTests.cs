...
namespace Ulearn.Core.Tests.Courses.Slides
{
	...
	public class SlideSerializationTests
	{
		...
		private static Slide DeserializeSlideFromXmlFile(string filename)
		{
			...
			var slide = slideFile.DeserializeXml<Slide>();
			return slide;
		}
		...
		{
			var slide = new Slide();
			var serializedSlide = slide.XmlSerialize();

			serializedSlide.DeserializeXml<Slide>();
		}
		...
		public void SerializeSlideWithoutBlocks()
		{
			...
			var deserializedSlide = serializedSlide.DeserializeXml<Slide>();
		...
		public void DeserializeSlideWithDifferentBlocks()
		{
			...
			CollectionAssert.AreEqual(new [] { "x = 1", @"a_i = \left{(}\frac{n(n+1)}{2}\right{)}" }, ((TexBlock)slide.Blocks[3]).TexLines);
			CollectionAssert.AreEqual(new [] { "image1.png", "image2.png" }, ((ImageGalleryBlock)slide.Blocks[4]).ImageUrls);
		}
	}
}