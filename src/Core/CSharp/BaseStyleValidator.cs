...
namespace uLearn.CSharp
{
	public abstract class BaseStyleValidator : ICSharpSolutionValidator
	{
		...
		*/
		...
		protected IEnumerable<SolutionStyleError> InspectAll<TNode>(SyntaxTree userSolution, SemanticModel semanticModel, Func<TNode, SemanticModel, IEnumerable<SolutionStyleError>> inspect)
		...
		protected IEnumerable<SolutionStyleError> InspectAll<TNode>(SyntaxTree userSolution, Func<TNode, IEnumerable<SolutionStyleError>> inspect)
			where TNode : SyntaxNode
		{
			var nodes = userSolution.GetRoot().DescendantNodes().OfType<TNode>();
